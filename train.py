...
file_root = 'data/allImage-voc2007-2012' # '/home/xzh/data/VOCdevkit/VOC2012/allimgs/'
learning_rate = 0.001
...
if use_resnet:
    ...
    for k in new_state_dict.keys():
        print(k)
        if k in dd.keys() and not k.startswith('fc'): ## 不加载全连接层的参数
            print('yes')
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for i,(images,target) in enumerate(test_loader):
            ...
            pred = net(images)
            loss = criterion(pred,target)
            validation_loss += loss.item() # data[0]
